...
SceneHierarchyPanel::SceneHierarchyPanel(EditorLayer* edl) : Panel(edl) 
{
...
void SceneHierarchyPanel::OnImGuiRender(float dt)
{
	...
	auto view = m_Edl->m_Registry.view<TagComponent>();
	bool otherPopup = false;
	for (auto entity : view)
	{
		auto tc = m_Edl->m_Registry.get<TagComponent>(entity);
		const auto nodeFlags = ImGuiTreeNodeFlags_SpanAvailWidth;

		//ImGui::Text("(You can also right-click me to open the same popup as above.)");
		//if (ImGui::TreeNode(name.Name,, m_Selected == entity))
		auto flags = m_Edl->m_Selected == entity ? ImGuiTreeNodeFlags_Selected : 0;
		//if (ImGui::TreeNodeEx(name.Name), flags)
		if (ImGui::Selectable(tc.Name, m_Edl->m_Selected == entity))
		{
			m_Edl->m_Selected = entity;
	...
	if (leftClick) m_Edl->m_Selected = entt::null;
	...
	if (ImGui::BeginPopup("ContextMenu"))
	{
		if (ImGui::MenuItem("Create Empty"))
		{
			...
			auto newEntity = m_Edl->m_Registry.create();
			m_Edl->m_Registry.emplace<TransformComponent>(newEntity);

			std::string* newName = new std::string("GameObject" + (count > 0 ? " (" + std::to_string(count) + ")" : ""));
			auto zzz = entt::hashed_string{ newName->c_str() };
			auto tc = m_Edl->m_Registry.emplace<TagComponent>(newEntity, zzz);
		}
		ImGui::EndPopup();
	}
	ImGui::End();
