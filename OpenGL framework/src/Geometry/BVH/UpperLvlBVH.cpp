...
void UpperLvlBVH::Buffer()
{
	if (m_BVH_SSBO) glDeleteBuffers(1, &m_BVH_SSBO);
	const unsigned int poolSize = sizeof(BVHNode) * m_BVHBuffer.size();
	glGenBuffers(1, &m_BVH_SSBO);
	glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_BVH_SSBO);
	glBufferData(GL_SHADER_STORAGE_BUFFER, poolSize, &m_BVHBuffer[0], GL_DYNAMIC_COPY);
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, m_BVH_SSBO);
	glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
}

void UpperLvlBVH::Draw(Camera& camera, glm::mat4 transform, BVHComponent& bvhc)
{
